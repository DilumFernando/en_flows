...
from .dynamics import InversedDynamics, DensityDynamics
...
class RegularizedHutchinsonEstimator(torch.nn.Module):
    def __init__(self, dynamics_function, noise=None, brute_force=False):
        super().__init__()
        self._dynamics_function = dynamics_function
        self._noise = noise
    ...
    def forward(self, t, xs):
        with torch.set_grad_enabled(True):
            ...
            assert len(
                dxs.shape) == 2, "`dxs` must have shape [n_btach, system_dim]"
            system_dim = dxs.shape[-1]
            assert self._brute_force == False
            if self._brute_force is True:
                divergence = 0.0
                for i in range(system_dim):
                    ddxsi_dxs = torch.autograd.grad(
                ...
                if self._reset_noise is True:
                    self._reset_noise = False
                    self._noise = torch.randint(low=0, high=2,
                                                size=xs.shape).to(xs) * 2 - 1
                noise_ddxs = \
                torch.autograd.grad(dxs, xs, self._noise, create_graph=True)[0]

                divergence = torch.sum(
                    (noise_ddxs * self._noise).view(-1, system_dim), 1,
                    keepdim=True)

                # Regularization terms:
                frobenius = noise_ddxs.pow(2).view(-1, system_dim).sum(dim=1,
                                                                       keepdim=True)
                dxs2 = dxs.view(-1, system_dim).sum(dim=1, keepdim=True)
            else:
                raise "Only Hutchinson and bruteforce are implemented."
        return dxs, divergence, frobenius, dxs2
